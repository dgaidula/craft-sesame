...
- **NEXT: P1.2 UI pass** (code-management CP screens: list codes w/ labels/expiry/per-code revoke+link+"Add code" on Pro; Lite keeps the single Password field — engine already supports it via codeOne; the Codes service is the whole API, UI touches no engine code), then **P1.3 branding** (CP-editable challenge logo/heading/intro/accent, site default + per-rule, plugin-table stored).
