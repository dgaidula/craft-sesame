...
- **★ ALL P0 + P1 COMPLETE.** P1.1 scheduled lock/unlock, P1.2 named codes (engine+UI), P1.3 branding all shipped + verified. Remaining = **P2 only on a demand signal** (reCAPTCHA port from Downtoll; leak-sealing helper craft.sesame.isProtected + query filter BOTH editions + soften README; anonymize-IP setting; bulk "protect selected" action). Plus release-prep before Store submission (migration baseline / schemaVersion, since schema was baked into Install.php while unreleased).
